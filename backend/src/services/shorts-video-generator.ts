...
async function generateNaturalAudio(
  ...
  try {
    ...
    const errorMsg = 'All audio generation methods failed. Tried: HTTP-based TTS (Google Translate).'
  ...
  try {
    ...
    if (!audioPath) {
      return {
        ...
        errorMessage: 'Failed to generate audio. Tried: HTTP-based TTS (Google Translate).'
      }
    }
